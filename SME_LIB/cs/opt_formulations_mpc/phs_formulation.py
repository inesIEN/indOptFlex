...
def phs_opt_formulation_energy_based_mpc(self,mdl,K,x_o,mode,from_agent,heating_demand_t,q_chp_in,Qh_hvac,n,u_past):
    ...
    N_h_b_t = mdl.addVars([(h, t) for h in H_binary for t in list(range(-self.tbs.phs.heat_pumps[h]["t_on"], 0)) + K],
    ...
    N_h_t = mdl.addVars([(h, t) for h in heaters for t in t_on+K], vtype=GRB.CONTINUOUS,ub=1,lb=0, name="N_h_t")
    ...
    #ON/OFF variables
    z_on = mdl.addVars([(h, t) for h in (H_binary) for t in list(range(-self.tbs.phs.heat_pumps[h]["t_on"], 0))+K], vtype=GRB.BINARY)
    z_off = mdl.addVars([(h, t) for h in (H_binary) for t in list(range(-self.tbs.phs.heat_pumps[h]["t_on"], 0)) + K], vtype=GRB.BINARY)
    ...
    if mode == "MILP":
        ...
        T_tes_ini = x_o["Ts_phs"]
        mdl.addConstrs((H_test_t[t] == (row_tes * V_tes) * c_tes * (T_tes_ini - T_amb[t]) / 3600 for t in K if t == 0))  # in KWH
        mdl.addConstrs(T_tes_t[t] == x_o["Ts_phs"] for t in K if t == 0)



    # Heat Energy  Flow in and heat flow out  of the tank
    #From MFS
    mdl.addConstrs(Qmfs_out_t[t] == heating_demand_t[t+n]  for t in K)
    #From Heat Pumps
    mdl.addConstrs(Qh_in_t[t] == quicksum(self.tbs.phs.heat_pumps[h]["p_kw"] * self.tbs.phs.heat_pumps[h]["cop"] *
    ...
    for h in H_binary:
        mdl.addConstrs(N_h_t[h, t] == u_past[h][t + self.tbs.phs.heat_pumps[h]["t_on"]]
                       for t in list(range(-self.tbs.phs.heat_pumps[h]["t_on"], 0)))

    for h in H_binary:
        mdl.addConstrs(N_h_t[h, t] - N_h_t[h, t - 1] == z_on[h, t] - z_off[h, t]
                       for t in list(range(-self.tbs.phs.heat_pumps[h]["t_on"], 0)) + K if
                       t > -self.tbs.phs.heat_pumps[h]["t_on"])

        mdl.addConstrs(z_on[h, t] + z_off[h, t] <= 1 for t in list(range(-self.tbs.phs.heat_pumps[h]["t_on"], 0)) + K)
    ...
    for h in H_binary:
            ...
            mdl.addConstrs(quicksum(z_on[h, t - j] for j in MR) <= 1 - z_off[h, t] for t in K)
            mdl.addConstrs(quicksum(z_off[h, t - j] for j in MO) <= 1 - z_on[h, t] for t in K)

    return N_h_t,p_phs_t,Pel_h,H_test_t,T_tes_t,Qh_in_t,Qmfs_out_t,Qchp_in_t,heaters
