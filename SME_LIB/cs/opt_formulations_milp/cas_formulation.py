...
def cas_opt_formulation(self,mdl,K,x_o,mode,from_agent,air_demand_opt):
    ...
    N_k_b_t = mdl.addVars([(k, t) for k in compressors for t in K], vtype=GRB.BINARY, ub=1, lb=0, name="N_k_b_t")
    ...
    N_k_c_t = mdl.addVars([(k, t) for k in compressors for t in K], vtype=GRB.CONTINUOUS, ub=1, lb=0, name="N_k_c_t")
    ...
    z_on = mdl.addVars([(k, t) for k in (compressors) for t in K], vtype=GRB.BINARY)
    z_off = mdl.addVars([(k, t) for k in (compressors) for t in K], vtype=GRB.BINARY)
    ...
    for k in K_binary:
        ...
        mdl.addConstrs(z_on[k, t] + z_off[k, t] <= 1 for t in K)
    ...
    for k in compressors:
        if k in K_binary:
            ...
            mdl.addConstrs(quicksum(z_on[k, t - j] for j in MR if t-j>0) <= 1 - z_off[k, t] for t in K)
            mdl.addConstrs(quicksum(z_off[k, t - j] for j in MO if t-j >0) <= 1 - z_on[k, t] for t in K)

    # Run time constraints using switching variables
    ...
    return p_cas_t,Pel_k,v_out,v_in,v_stor_t,p_t,N_k_t,compressors

def cas_opt_results(self,K,p_cas_t,N_k_t,Pel_k,v_stor_t,v_in,v_out,p_t):
    compressors = list(self.tbs.cas.compressors.keys())
